...
pdf.ln(5)
escreverParagrafo('Organizando todas as cidades em seus devidos departamentos e analisando a taxa de mortalidade geral do departamento, foi possível chegar ao resultado do gráfico abaixo:')
pdf.image('departamentos.png', w=113, h=62, x=48.5)
escreverParagrafo('Como se pode ver o departamento com maior taxa de letalidade por COVID-19 foi o de '+main.dadosDRSLetal[0]+' com '+str(main.dadosDRSLetal[1]).replace('.',',')+'%'+' de letalidade pórem, considerando que pessoas são dados inteiros, sempre que o valor em uma casa decimal passar de 0 é necessário arredondar "para cima" nesse caso '+str(round(main.dadosDRSLetal[1])+1)+'% é a taxa de letalidade real.')
pdf.ln(5)
